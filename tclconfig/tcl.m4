...
AC_DEFUN([TEA_CONFIG_CFLAGS], [
	...
	Darwin-*)
	    ...
	    dnl AC_CACHE_CHECK([if ld accepts -single_module flag], tcl_cv_ld_single_module, [
	    dnl 	hold_ldflags=$LDFLAGS
	    dnl 	LDFLAGS="$LDFLAGS -dynamiclib -Wl,-single_module"
	    dnl 	AC_LINK_IFELSE([AC_LANG_PROGRAM([[]], [[int i;]])],
	    dnl 		[tcl_cv_ld_single_module=yes],[tcl_cv_ld_single_module=no])
	    dnl 	LDFLAGS=$hold_ldflags])
	    dnl AS_IF([test $tcl_cv_ld_single_module = yes], [
	    dnl 	SHLIB_LD="${SHLIB_LD} -Wl,-single_module"
	    dnl ])
